...
(ert-deftest test-help-find-org ()
  ...
  (should (string-equal (file-name-nondirectory (buffer-file-name)) (format "%s.el" help-find-org-test-lib)))
  (kill-buffer)
  ...
  (help-find-org 1)
  (find-function help-find-org-test-fn)
  (should (string-equal (file-name-nondirectory (buffer-file-name)) (format "%s.org" help-find-org-test-lib)))
  )
